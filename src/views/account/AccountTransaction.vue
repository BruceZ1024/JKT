    ...
    <template #right-icon>
      <i>
        <svg-icon icon-class='copy' style='width:13px; height:13px;' class="right-icon-account"
                  @click="copyToClipboard($event, 'Address: dfasdouewoiruoqiwer798')"></svg-icon>
      </i>
    </template>
  </van-cell>
  <van-tabs v-model:active="activeName">
    <van-tab title="Withdraw" name="Withdraw">
      <van-list v-model:loading="withdrawLoading" :finished="withdrawFinished" finished-text="No more item"
                @load="withdrawOnLoad">
    ...
    <van-tab title="Deposit" name="Deposit">
      <van-list v-model:loading="depositLoading" :finished="depositFinished" finished-text="No more item"
                @load="withdrawOnLoad">
        <van-cell-group v-for="item in list" :key="item.key" inset class="van-cell-group-margin">
          <van-cell class="padding-bottom-0" center :title="'+ '+item.number" :label="item.account">
            <template #icon>
