...
<van-cell title="" :label="'Address: '+ userAddress">
    <template #right-icon>
        <i>
            <svg-icon icon-class='copy' style='width:13px; height:13px;' class="right-icon-account" @click="copyToClipboard($event, userAddress)"></svg-icon>
        </i>
    </template>
</van-cell>
<van-tabs v-model:active="activeName">
    <van-tab title="Withdraw" name="Withdraw">
        <van-list v-model:loading="withdrawLoading" :finished="withdrawFinished" finished-text="No more" @load="withdrawOnLoad">
                ...
                <van-cell class="padding-bottom-0" center :title="'+ '+item.amount" :label="item.tx_id">
                    ...
                    <template #right-icon>
                        <div><span>JKT</span>
                            <br />
                            <i>
                                <svg-icon icon-class='small-copy' style='width:13px; height:13px;' class="right-icon-account" @click="copyToClipboard($event, item.account)"></svg-icon>
                            </i>
                        </div>
                    </template>
    ...
    <van-tab title="Deposit" name="Deposit">
        <van-list v-model:loading="depositLoading" :finished="depositFinished" finished-text="No more" @load="depositOnLoad">
