<template>
  <van-cell center title="$ 34221" label="ID: 342324">
    <template #title>
      $ 34221
      <svg-icon :icon-class="showAmount ? 'show' : 'hidden'" @click="toggleShow()"
                style='width:16px; height:16px;' class="right-icon-account"></svg-icon>
    </template>
    <template #icon>
    ...
    <template #right-icon>
      <van-button type="danger" class="btn-small-account-min-width"
                  @click="goTo('/account/transaction')" size="small">Transactions
      </van-button>
    </template>
  ...
  <van-cell title="" label="Address: dfasdouewoiruoqiwer798">
    <template #right-icon>
      <svg-icon icon-class='copy' style='width:13px; height:13px;'
                class="right-icon-account"></svg-icon>
    </template>
  </van-cell>
  ...
  <van-cell-group inset class="van-cell-group-margin">
    <van-cell center title="VIP Status">
      ...
      <template #right-icon>
        <van-button type="danger" class="btn-small-account-min-width"
                    @click="goTo('/account/upgrade')" size="small">Upgrade
        </van-button>
      </template>
    </van-cell>
  </van-cell-group>
  <van-cell-group inset class="van-cell-group-margin">
    <van-cell center title="JKT" label="3825.44">
      <template #icon>
        <svg-icon icon-class='small-jkt' style='width:34px; height:34px;'
                  class="right-icon-account"></svg-icon>
      </template>
      <template #right-icon>
        <van-button plain size="small"
                    class="btn-small-account-min-width right-icon-account btn-plain-red-white-dark"
                    @click="showWithdraw = true">Withdraw
        </van-button>
        <van-button class="btn-small-account-min-width" type="danger" size="small"
                    @click="showDeposit = true">Deposit
        </van-button>
      </template>
    </van-cell>
    <!-- Deposit pop up -->
    <van-popup :show="showDeposit" round position="bottom" closeable>
      ...
      <van-cell-group class="van-popup-fixed-height">
        <van-cell-group inset class="account-cell-group-dark ">
          <van-field class="account-cell-dark" v-model="amountDeposit" center clearable label=""
    ...
    <!-- Withdraw pop up -->
    <van-popup class="van-popup-fixed-height" :show="showWithdraw" round position="bottom"
               closeable>
      ...
      <van-cell-group class="van-popup-fixed-height">
        ...
        <div class="account-label-container">
          <p class="success-label">
            Wallet Balance: 3423,34 JKT</p>
          <p class="success-label">
            Transaction Fees: 8 % (7% will be transferred to the black hole address for
            destruction)</p>
        </div>
        <van-cell>
  ...
  <van-cell-group inset class="van-cell-group-margin">
    <van-cell center title="Invite Friends">
      <template #icon>
        <svg-icon icon-class='invite-friends' style='width:34px; height:34px;'
                  class="right-icon-account"></svg-icon>
      </template>
      <template #right-icon>
        <van-button class="btn-small-account-min-width" type="danger" size="small"
                    @click="goTo('/account/invite')">Invite
        </van-button>
      </template>
    </van-cell>
    ...
    <van-cell center title="BSC Contract">
      <template #icon>
        <svg-icon icon-class='bsc-contract' style='width:34px; height:34px;'
                  class="right-icon-account"></svg-icon>
      </template>
      <template #right-icon>
    ...
    <van-cell center title="Contact Us" is-link icon="phone-o">
      <template #icon>
        <svg-icon icon-class='contact-us' style='width:34px; height:34px;'
                  class="right-icon-account"></svg-icon>
      </template>
    </van-cell>
    <van-divider :style="{ borderColor: '#FFFFFF', padding: '0 16px' }"></van-divider>
    <van-cell center title="Settings" is-link icon="setting-o" @click="goTo('/account/setting')">
      <template #icon>
        <svg-icon icon-class='settings' style='width:34px; height:34px;'
                  class="right-icon-account"></svg-icon>
      </template>
    </van-cell>
  </van-cell-group>
